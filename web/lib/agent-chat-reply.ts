...
function matchesStrategy(t: string): boolean {
  return /\b(strategy|preference|principles|playstyle)\b/i.test(t);
}

function matchesRisk(t: string): boolean {
  return /\b(risk|scam|blind\s*swap|fraud)\b/i.test(t);
}

function matchesReputation(t: string): boolean {
  return /\b(reputation|reviews?|behavioral|verified|social)\b/i.test(t);
}

function matchesTrade(t: string): boolean {
  return /\b(swap|trade|proposal|inquiry|budget|round)\b/i.test(t);
}

function matchesDeception(t: string): boolean {
  return /\b(lie|lying|exaggerat|deceive)\b/i.test(t);
}

function matchesGreeting(t: string): boolean {
  return /^(hi|hello|hey)\b/i.test(t.trim());
}
...
export function generateAssistantReply(input: ReplyInput): string {
  ...
  const lowDeception = p.deceptionWillingness <= 45;
  const riskCautious = p.riskPreference <= 45;
  const repCareful = p.reputationSensitivity >= 55;
  const chatty = p.socialActivity >= 55;
  const patient = p.urgencyThreshold <= 45;

  if (matchesStrategy(userText)) {
    if (lowDeception && riskCautious) {
      return pick([
    ...
    return pick([
      ...
      "I'll balance 'ask more' vs 'swap fast' using your urgency and social-activity lean.",
    ]);
  }
  ...
  if (matchesTrade(userText)) {
    ...
    return pick([
      "Each inquiry costs budget; one swap proposal per loop—I'll spend along your social/urgency mix.",
